################################################################################
#After processing with local blockchain data, revisit the database and fill in 
#   blame record with information only accessible remotely.
...
NUM_CONSECUTIVE_BLOCKS_TO_CLAIM = 100
#NUM_BLOCKS_TO_JUMP_IF_COLLISION = 100
...
if ENABLE_MULTITHREADING:
    ...
    if len(dead_blocks) > 0:
        print ("Error: The following blocks were claimed by workers that "
               "appear to have crashed, and must be manually reclaimed. If you "
               "don't know how to do this, file an issue on GitHub.")
...
if ENABLE_MULTITHREADING:
    ...
    starting_height=lowest_height_with_def_records, num_to_claim=NUM_CONSECUTIVE_BLOCKS_TO_CLAIM)
    try:
...
if current_height_iterated is None:
    print("No deferred records found in database. TODO: Implement data subscription.")
    sys.exit()
...
try:
    ...
    #   "update" scripts. Actually, I don't think there's any rollback to do.
